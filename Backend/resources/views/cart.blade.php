        ...
        <div class="container">
            ...
            <div class="cart-container">
                ...
                <div class="order-summary" id="order-summary-box">
                    ...
                    <div class="summary-item"><span>Subtotal (Excl. delivery)</span><span id="summary-subtotal">Rs 0.00</span></div>
    ...
    <script>
        ...
        async function fetchCart() {
            try {
                ...
                document.getElementById('cart-items-container').innerHTML = '<p style="text-align:center; color:red;">Failed to load cart. Please refresh the page.</p>';
            }
        }
        ...
        function renderCart(data) {
            ...
            let subtotal = data.subtotal || cart.subtotal || 0;

            // Always recalculate client-side to ensure accuracy
            const calculatedSubtotal = items.reduce((sum, item) => sum + (parseFloat(item.total_price) || 0), 0);

            // Use server subtotal if available and matches calculation, otherwise use calculated
            if (subtotal > 0 && Math.abs(subtotal - calculatedSubtotal) < 0.01) {
                // Server calculation matches
            } else if (calculatedSubtotal > 0) {
                console.warn('Using client-side calculated subtotal:', calculatedSubtotal, 'server subtotal:', subtotal);
            ...
            document.getElementById('summary-subtotal').textContent = `Rs ${parseFloat(subtotal).toFixed(2)}`;
